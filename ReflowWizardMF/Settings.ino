...
// of 175 to 280.
//
// For the baking temperature, we just use 8-bit. No known parts need bake at temperature above 255.
...
void InitializeSettingsIfNeccessary() {
  ...
  if (getSetting(SETTING_EEPROM_NEEDS_INIT)) {
    ...
    // set baking defaults
    setSetting(SETTING_BAKE_TEMP,BAKE_TEMP);
    setSetting(SETTING_BAKE_DURATION,BAKE_DURATION);
  }
